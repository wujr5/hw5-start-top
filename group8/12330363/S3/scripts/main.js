...
(function(){
  var Button, setAllButtons, setLargeBubble, setApb, robot, clickButtonsParallelly;
  Button = (function(){
  ...
  clickButtonsParallelly = function(){
    $('#button .apb').click(function(){
      var i$, ref$, len$;
      Button.thisButtonClickCallback = function(){
        if (Button.checkAllButtons()) {
          robot.largeBubble.click();
        }
      };
      for (i$ = 0, len$ = (ref$ = robot.buttons).length; i$ < len$; ++i$) {
        (fn$.call(this, ref$[i$]));
      }
      function fn$(btn){
        btn.click();
