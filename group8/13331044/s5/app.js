...
(function(){
  ...
  Button = (function(){
    ...
    function Button(dom, goodMessage, badMessage, feedback){
      var this$ = this;
      this.dom = dom;
    ...
    prototype.successOrFail = function(number){
      if (Math.random() > 0.5) {
        bubble.showMessage(this.name, this.goodMessage);
        this.feedback(null, number);
      } else {
        this.feedback({
  ...
  addClickingToFetchNumbersToButtons = function(next){
    var goodMessage, badMessage, i$, ref$, len$;
    goodMessage = ['这是个天大的秘密', '我不知道', '你不知道', '他不知道', '才怪'];
    badMessage = ['这不是个天大的秘密', '我知道', '你知道', '他知道', '才怪'];
    for (i$ = 0, len$ = (ref$ = $('#control-ring .button')).length; i$ < len$; ++i$) {
      (fn$.call(this, i$, ref$[i$]));
    }
    function fn$(i, dom){
      var button;
      button = new Button($(dom), goodMessage[i], badMessage[i], function(error, number){
        if (error) {
          console.log(button.name + ": " + error.message);
        }
        bubble.sum += parseInt(number);
      });
    }
  };
  bubble = {
    ...
    showMessage: function(name, message){
      this.obj.find(".word").text(name + ": " + message);
    },
    clickToSumAndShow: function(){
      this.obj.find('.sum').text(this.sum);
    },
    reset: function(){
      this.sum = 0;
      this.msg.text('');
